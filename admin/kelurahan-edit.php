...
$id = (int)($_GET['id'] ?? 0);
if (!$id) redirect(SITE_URL . '/admin/kelurahan.php');

$stmt = $conn->prepare("SELECT * FROM kelurahan WHERE id = ?");
$stmt->bind_param('i', $id);
$stmt->execute();
$k = $stmt->get_result()->fetch_assoc();
if (!$k) redirect(SITE_URL . '/admin/kelurahan.php');

// Security: Hanya superadmin yang boleh edit kelurahan lain, 
// atau admin kelurahan hanya boleh edit datanya sendiri
if (!isSuperAdmin() && $_SESSION['kelurahan_id'] != $id) {
    setFlash('error', 'Anda tidak memiliki hak akses untuk mengedit wilayah ini.');
    redirect(SITE_URL . '/admin/index.php');
}

$pageTitle = 'Edit Kelurahan - ' . $k['nama'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama)) {
        ...
        $foto_sekretaris = $k['foto_sekretaris'];
        $foto_bendahara = $k['foto_bendahara'];
        $foto_pokja1 = $k['foto_pokja1'];
        $foto_pokja2 = $k['foto_pokja2'];
        $foto_pokja3 = $k['foto_pokja3'];
        $foto_pokja4 = $k['foto_pokja4'];
        ...
        foreach($uploads as $key => $upInfo) {
            if(!empty($_FILES[$key]['tmp_name'])){
                ...
                if(is_array($res) && isset($res['error'])){
                    ...
                    if(${$key} && file_exists($upInfo['dir'].'/'.${$key})) unlink($upInfo['dir'].'/'.${$key});
                    ${$key} = $res;
                }
            }
        }
        ...
        if(empty($error)){
            ...
            if ($stmt) {
                $stmt->bind_param('sssssssssssssssssssiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii', 
                    $nama, $ketua_pkk, $sekretaris_pkk, $bendahara_pkk,
                    $pokja1_pkk, $pokja2_pkk, $pokja3_pkk, $pokja4_pkk,
                    $foto_ketua, $foto_sekretaris, $foto_bendahara,
                    $foto_pokja1, $foto_pokja2, $foto_pokja3, $foto_pokja4,
                    ...
                    $bayi_lahir_l, $bayi_lahir_p, $akte_ada, $akte_tidak,
                    $bayi_meninggal_l, $bayi_meninggal_p, $balita_meninggal_l, $balita_meninggal_p,
                    $rumah_sehat, $rumah_kurang_sehat, $sampah, $jamban, $air_bersih, $id);
                
                if ($stmt->execute()) {
                    setFlash('success', 'Data kelurahan berhasil diperbarui!');
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    <div>
        ...
        <h2 class="text-2xl md:text-3xl font-bold text-darkblue_alt">Edit Kelurahan / RW</h2>
        <p class="text-gray-400 text-sm mt-1">Memperbarui data profil: <span class="font-bold text-darkblue_alt"><?= e($k['nama']) ?></span></p>
    </div>
...
<form method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="p-8 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama" required value="<?= e($_POST['nama'] ?? $k['nama']) ?>"
                ...
                <div class="bg-softgray/50 rounded-2xl p-5 border border-gray-100 group hover:border-accent transition-all">
                    <div class="flex items-center justify-between mb-3">
                        <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider"><?= $label ?></label>
                        <?php if(!empty($k[$file])): ?>
                            <div class="w-8 h-8 rounded-full border border-accent overflow-hidden shadow-sm">
                                <img src="<?= getImg($k[$file], 'bidang') ?>" class="w-full h-full object-cover">
                            </div>
                        <?php endif; ?>
                    </div>
                    <input type="text" name="<?= $name ?>" value="<?= e($_POST[$name] ?? $k[$name]) ?>" placeholder="Nama Lengkap"
                ...
                <div class="bg-softgray/50 rounded-2xl p-5 border border-gray-100 md:col-span-2">
                    <div class="flex items-center gap-4">
                        <?php if(!empty($k['gambar'])): ?>
                            <img src="<?= getImg($k['gambar'], 'kegiatan') ?>" class="w-24 h-24 rounded-xl object-cover border-2 border-accent/20 shadow-sm">
                        <?php endif; ?>
                        <div class="flex-1">
                            <label class="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-2">Ganti Foto Wilayah</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="deskripsi" rows="4"
                              class="w-full bg-softgray border border-gray-200 text-darkblue_alt rounded-2xl px-5 py-4 text-sm focus:outline-none focus:border-accent focus:ring-4 focus:ring-accent/10 transition-all resize-none"><?= e($_POST['deskripsi'] ?? $k['deskripsi']) ?></textarea>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="p-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div class="bg-softgray/30 rounded-2xl p-4 border border-gray-50 group hover:bg-white hover:border-accent transition-all">
                    ...
                    <input type="number" name="<?= $name ?>" value="<?= e($k[$name]) ?>"
                ...
                <div class="bg-accent/5 rounded-2xl p-4 border border-accent/20 md:col-span-2">
                    <div class="flex items-center gap-2 mb-2 text-accent">
                        ...
                        <label class="text-[10px] font-bold uppercase tracking-wider">Total Penduduk</label>
                    </div>
                    <input type="number" name="penduduk" value="<?= e($k['penduduk']) ?>"
        ...
        <div class="p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="makanan_pokok" value="<?= e($k['makanan_pokok']) ?>" placeholder="Contoh: Beras, Jagung..."
                ...
                <div>
                    ...
                    <textarea name="inovasi" rows="4" placeholder="Ceritakan program inovasi atau prestasi wilayah ini..."
                              class="w-full bg-softgray border border-gray-200 text-darkblue_alt rounded-2xl px-5 py-4 text-sm focus:outline-none focus:border-accent transition-all resize-none"><?= e($k['inovasi']) ?></textarea>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center gap-4 bg-white/50 backdrop-blur-sm sticky bottom-8 p-4 rounded-3xl border border-white shadow-2xl z-10">
        <button type="submit" class="flex-1 flex items-center justify-center gap-3 bg-accent hover:bg-darkblue text-white font-black px-8 py-5 rounded-2xl transition-all shadow-xl shadow-accent/20 uppercase tracking-widest text-sm">
            <i class="fas fa-save"></i> Perbarui Seluruh Data
        </button>
